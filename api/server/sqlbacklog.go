...
const (
	...
	dropTb   = "DROP TABLE IF EXISTS " + dbName
	createTb = `
...
func NewSqlBacklog(pass string) (backlog *sqlBacklog, err error) {
...
func (b *sqlBacklog) openDb(url string) {
...
func (b *sqlBacklog) GetHistory() (history []Message, err error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(nil, &m.Name, &m.Message, &timestamp); err != nil {
			return nil, err
		}
